...
class sodapop {
    ...
    public function parseUrl($part) {
        ...
        list($this->domain, $this->appUrl)	= array_pad(explode($this->config['liveSite'], $uri, 2), -2, null);;
        
        ## Check for a querie string in the handle
        if (strpos($this->appUrl, '?') !== false) {
            
            ## Separate the apps handle from the query string
            list($this->handle, $this->queryString)	= array_pad(explode("?", $this->appUrl, 2), -2, null);
        }
        
        ## If there is no querie string than the handle stands alone.
        else $this->handle    = $this->appUrl;
    ...
    public function modsInPosition($position) {
        ...
        foreach ($modsInPostionData as $modInPostionData) {
            ...
            foreach ($params as $k) {
                ...
                list ($name, $value)	= array_pad(explode("==", $k, 2), -2, null);
                $mod[$i][$name]			= $value;
    ...
    public function loadTest($fileName) {
        
        if($this->config['testingMode'] == "yes") {
            
            require_once($this->config['sitePath'] . "testing/" . $fileName .'_test.php');
